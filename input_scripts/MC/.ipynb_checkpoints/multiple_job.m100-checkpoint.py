### Python script to manage multiple simulations.
### Note: it could be much improved
### Conceived for CINECA supercomputing facilities, but can be easily adapted

import os
import string
import numpy as np

#t=(1.00e-07,5.00e-07,1.00e-06,2.5e-06,5e-06,1.00e-05,2.5e-05,5e-05,1.00e-04,5e-04,1.00e-03,5e-03)
t=(5.00e-03,)
npoint = len(t)
...
for i in range(npoint):
	...
	s = s.replace('$TEMP', format(t[i]))
	f = open(path + '/input.dat', 'w')
	f.write(s)
	f.close()
	
	s = open(path + '/input.dat').read()
	s = s.replace('$DELTA00', format(delta00[t[i]]))
	f = open(path + '/input.dat', 'w')
	f.write(s)
	f.close()

	s = open(path + '/input.dat').read()
	s = s.replace('$DELTA20', format(delta20[t[i]]))
	f = open(path + '/input.dat', 'w')
	f.write(s)
	f.close()

	s = open(path + '/input.dat').read()
	s = s.replace('$DELTA50', format(delta50[t[i]]))
	f = open(path + '/input.dat', 'w')
	f.write(s)
	f.close()

	s = open(path + '/input.dat').read()
	s = s.replace('$DELTAR', format(deltaR[t[i]]))
	f = open(path + '/input.dat', 'w')
	f.write(s)
	f.close()
	
	s = open(path + '/input.dat').read()
	s = s.replace('$DELTAB', format(deltaB[t[i]]))
	f = open(path + '/input.dat', 'w')
	f.write(s)
	f.close()

	s = open(path + '/input.dat').read()
	s = s.replace('$DELTACM', format(deltaCM[t[i]]))
	f = open(path + '/input.dat', 'w')
	f.write(s)
	f.close()	
